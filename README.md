...
| `GET /api/v1/branches/:branchId/reports/series` | VIEW_REPORTS | Takings per day over a run of shop-local days, for the chart. `?days=` (1–90, default 14) ending on `?date=`. One point per day **with no gaps** — a day the shop sold nothing on is a zero |
...
**A number on its own cannot say whether it is a good number.** `TSh 203,000`
answers "how much" and leaves "is that normal" — which is the question a
shopkeeper actually opened the page to ask — entirely unanswered. So Ripoti
draws the fortnight behind the figure, and the takings tile carries a
sparkline and a comparison against the run's own average rather than against
...
it, by construction rather than by two pieces of code agreeing by luck. A
`reports.e2e-spec.ts` test asserts the two match to the shilling. The
comparison itself is a pure function in `web/src/lib/series.ts`, and it
deliberately **refuses to compare** against a run averaging zero: a shop
reopening after a closed week should not be told its takings are up several
thousand percent.
